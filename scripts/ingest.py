...
def extract_profile(ds, profile_idx=0):
    """Extracts metadata and measurements for one profile."""
    profile = {}

    # Float ID
    profile['float_id'] = str(
        ds.attrs.get('PLATFORM_NUMBER', '') or 
        ds.attrs.get('platform_number', '') or 
        ds.attrs.get('WMO_ID', '')
    )
    if not profile['float_id'] and 'FLOAT_ID' in ds:
        profile['float_id'] = str(ds['FLOAT_ID'].values)
    ...
    profile['lat'] = float(get_var('LATITUDE', get_var('LAT', np.nan)))
    profile['lon'] = float(get_var('LONGITUDE', get_var('LON', np.nan)))
    profile['cycle_number'] = int(get_var('CYCLE_NUMBER', -1))
    ...
    measurements = {}
    qc_summary = {}
    var_map = {'PRES':['PRES','DEPH'], 'TEMP':['TEMP','TEMPF'], 'PSAL':['PSAL','SALT'], 'DOXY':['DOXY','OXY']}
        ...
        for name in candidates:
            if name in ds.data_vars:
                ...
                if qc_name in ds.data_vars:
                    ...
                    good_flags = np.sum((qc_vals==1)|(qc_vals==b'1'))
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("""
            ...
            VALUES(:pid,:fid,:cyc,:date,:lat,:lon,ST_GeomFromText(:geom,4326),
                   :n_levels,:min_pres,:max_pres,:qc::jsonb,:summary,:parquet,:src,:cs,:ver)
